...
const appStart$ = (action$: any, getState: Function) => {
  ...
  return Observable.from(getProjectList(credentials))
    .switchMap((d) => {
      return Observable.of(loginSuccess(credentials, userInfo), appStarted(), setInitData(d));
    })
    .takeUntil(action$.ofType('APP_STOP'))
    .catch((err) => {
      console.error(['API call failed', err]);
      if (err instanceof PermissionError) {
        return Observable.of(sessionInvalid(err), appStarted());
      }
      return Observable.of(appError(appMessages.errorsServer, err));
    });
};

export const RehydrateEpic = (action$: any, { getState }: Deps) =>
  action$.ofType(REHYDRATE).concatMap(() => {
    // Run the start epic after rehydrate finished
    const { starting } = getState().app;
    if (!starting) {
      return Observable.of(appRehydrated());
    }
    return Observable.concat(Observable.of(appRehydrated()), appStart$(action$, getState));
  });

export const appStartEpic = (action$: any, { getState }: Deps) =>
  action$.ofType('APP_START').concatMap(() => {
    // Run the start epic after rehydrate finished
