...
class DataDogSetting extends React.PureComponent {
  props: Props;
  state: States = {
    appKey: '',
    apiKey: '',
  ...
  @autobind handleRegisterClick(e) {
    ...
    const { appKey, apiKey, samplingInterval } = this.state;
    this.props.createProject(projectName, 'DataDog', {
      apiKey,
      appKey,
      samplingInterval,
    });
  ...
  render() {
    ...
    const appKeyLink = VLink.state(this, 'appKey').check(x => x, 'The app key is required');
    const apiKeyLink = VLink.state(this, 'apiKey').check(x => x, 'The api key is required');
    const hasError = samplingIntervalLink.error || appKeyLink.error || apiKeyLink.error;
