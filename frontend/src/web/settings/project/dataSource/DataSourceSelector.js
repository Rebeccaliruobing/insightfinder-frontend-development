...
class DataSourceSelector extends React.PureComponent {
  ...
  @autobind toggleDataSourceSelected(name) {
    return () => {
      const { selectedDataSources, onSelectionChange, radioMode } = this.props;
      // If the data source already exists, remove it from the selection.
      const index = R.findIndex(d => d === name, selectedDataSources);
      let dataSources = [];
      if (radioMode) {
        dataSources = index >= 0 ? [] : [name];
      } else {
        dataSources = index >= 0
          ? R.remove(index, 1, selectedDataSources)
          : [...selectedDataSources, name];
      }
  ...
  render() {
    ...
    return (
      <div className={`${className} flex-col`}>
        <div className="flex-row hide" style={{ marginBottom: '1em' }}>
              ...
              return (
                <Tile size={12} key={name}>
                  ...
                  >
                    {configured &&
                      <div
                        className="ui fitted disabled checkbox"
                    ...
                    <div style={{ paddingLeft: 28 }}>
                      ...
                      <div
                        className="desc"
                        dangerouslySetInnerHTML={{ __html: intl.formatMessage(desc) }}
                      />
                    </div>
                  </Box>
                </Tile>
