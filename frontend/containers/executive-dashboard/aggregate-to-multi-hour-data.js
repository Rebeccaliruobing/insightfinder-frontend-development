const aggregateToMultiHourData = (jsonResponse = {}, durationInDays = 7, periodInHours = 3, endTimestamp = new Date().getTime()) => {
  let d = new Date();
  const viewMillisOffset = d.getTimezoneOffset() * 60 * 1000; 
  const millisInDay = 24 * 60 * 60 * 1000;
  const millisInHour = 60 * 60 * 1000;
  let earliestDataHour = topOfDay(endTimestamp - (durationInDays * millisInDay));
  let earliestViewHour = 0;
