...
class SettingDebug extends React.Component {
    ...
    render() {
        ...
        timeMockup = (_.keysIn(timeMockup)).length != 0 ? timeMockup : [];
        freqMockup = (_.keysIn(freqMockup)).length != 0 ? freqMockup : [];
                        ...
                        <div className="content">
                            <div style={{'width': '100%','display': 'flex'}}>
                                {_.keysIn(dataArray).map(function (value, index) {
                                    ...
                                    return (
                                        <PieChart pieChartCanvasStyle={pieChartCanvasStyle} radius={[55,80]} key={index}
                                                  optionCenterStyle={optionCenterStyle} colorChart="#3398DB"
                                                  titleStyle={titleStyle} data={value}
                                                  dataValue={dataValue.toString()}/>);
                                })}
                            </div>
                            {
                                (timeMockup.length == 0 && freqMockup.length == 0) ? null :
