...
const ProjectDetails = class extends React.Component {
  ...
  updateData() {
    ...
    let {projectName, startTime, endTime, startTimestamp, endTimestamp, groupId, instanceName, metricName, anomalyMetrics} = query;
    let refreshInterval = parseInt(store.get(ChartsRefreshInterval, 0));
    this.setState({loading: true}, ()=> {
      apis.postProjectData(projectName, startTime, endTime, startTimestamp, endTimestamp, groupId, instanceName, metricName, anomalyMetrics)
        .then(resp => {
          let update = {};
          if (resp.success) {
