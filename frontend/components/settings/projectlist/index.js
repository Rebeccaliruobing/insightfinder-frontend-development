...
class Projects extends React.Component {
  ...
  constructor(props) {
    ...
    this._el = null;
    let tabStates = ['custom', 'amazon', 'google', 'datadog', 'newrelic'].indexOf(
  ...
  projectList(projectString, projectGroupByType, projectInfoArray) {
    ...
    if (projectString.length > 0) {
      const projects = JSON.parse(projectString || '[]');
      R.forEachObjIndexed((project) => {
        const {projectName: name, projectType, cloudType, dataType} = project;
        let zone = projectInfoArray.find(pair => pair[0] == name)
          ? projectInfoArray.find(pair => pair[0] == name)[1]
          : 'N/A';
        let agentDataEnabled = projectInfoArray.find(pair => pair[0] == name)
          ? projectInfoArray.find(pair => pair[0] == name)[2]
          : false;
        let instanceType = projectInfoArray.find(pair => pair[0] == name)
          ? projectInfoArray.find(pair => pair[0] == name)[3]
          : false;
        switch (projectType) {
          case 'AWS':
          case 'EC2':
          case 'RDS':
          case 'DynamoDB':
            projectGroupByType.AWS.push({
              name,
              projectType: instanceType,
              cloudType,
              dataType,
              zone,
              agentDataEnabled,
          ...
          case 'GAE':
          case 'GCE':
            projectGroupByType.Google.push({
              name,
              projectType,
              cloudType,
              dataType,
          ...
          default:
            switch (cloudType) {
              case 'DataDog':
                projectGroupByType.datadog.push({
                  name,
                  projectType,
                  cloudType,
                  dataType,
                ...
                break;
              case 'NewRelic':
                projectGroupByType.newrelic.push({
                  name,
                  projectType,
                  cloudType,
                  dataType,
                ...
                break;
              default:
                projectGroupByType.custom.push({
                  name,
                  projectType,
                  cloudType,
                  dataType,
  ...
  render() {
    ...
    let {
      ...
      incidentAllInfo,
      dataAllInfo,
      projectSettingsAllInfo,
    } = this.context.dashboardUservalues;
    let projectInfoArray = projectSettingsAllInfo.map(s => [
      s.projectName,
      s.zone,
      s.agentDataEnabled,
      s.instanceType,
      ...
      <Console.Content className="projects">
        <div className="ui main tiny container" ref={c => this._el = c}>
          <div className="ui pointing secondary menu">
            <a className={`${tabStates.custom} item`} onClick={e => this.selectTab(e, 'custom')}>
              Insight Agent
            </a>
            <a className={`${tabStates.amazon} item`} onClick={e => this.selectTab(e, 'amazon')}>
              AWS CloudWatch
            </a>
            <a className={`${tabStates.google} item`} onClick={e => this.selectTab(e, 'google')}>
              Google Cloud Monitoring
            </a>
            <a
              className={`${tabStates.datadog} item`}
              onClick={e => this.selectTab(e, 'datadog')}
            >
              DataDog
            </a>
            <a
              className={`${tabStates.newrelic} item`}
              onClick={e => this.selectTab(e, 'newrelic')}
            >
              New Relic
            </a>
          </div>
          <div className={`${tabStates.amazon} ui tab `}>
            {tabStates.amazon === 'active'
              ? <AmazonProjects projects={projectGroupByType.AWS} />
              : null}
          </div>
          <div className={`${tabStates.google} ui tab `}>
            {tabStates.google === 'active'
              ? <ProjectsGoogle projects={projectGroupByType.Google} />
              : null}
          </div>
          <div className={`${tabStates.custom} ui tab `}>
            {tabStates.custom === 'active'
              ? <CustomProjects projects={projectGroupByType.custom} />
              : null}
          </div>
          <div className={`${tabStates.datadog} ui tab `}>
            {tabStates.datadog === 'active'
              ? <DataDogProjects projects={projectGroupByType.datadog} />
              : null}
          </div>
          <div className={`${tabStates.newrelic} ui tab `}>
            {tabStates.newrelic === 'active'
              ? <NewRelicProjects projects={projectGroupByType.newrelic} />
              : null}
          </div>
        </div>
      </Console.Content>
